...
// Delete an existing note given the id
app.delete("/deleteNote/:noteId", express.json(), async (req, res) => {
  try {
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      // Find note with given ID and delete it
      const collection = db.collection(COLLECTIONS.notes);
      ...
      if (data.deletedCount === 0) {
        // Note with given ID not found (already deleted)
        return res
          .status(404)
...
app.patch("/editNote/:noteId", express.json(), async (req, res) => {
  try {
    ...
    // Find note with given ID belonging to the user
    const collection = db.collection(COLLECTIONS.notes);
    ...
    // Partially update the note based on request body
    const { title, content } = req.body;
    if (title !== undefined) {
    ...
    await collection.updateOne(
      { _id: new ObjectId(noteId) },
      { $set: { title: existingNote.title, content: existingNote.content }}
    );
    res.json({
